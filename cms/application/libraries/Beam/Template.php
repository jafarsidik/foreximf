		...
		foreach ($options as $key => $value)
		{
			switch ($key)
			{
				...
				case 'default_layout':
					...
					break;
				case 'front_end':
					$this->front_end = $this->layout = $value;
					break;
				default:
					$this->$key = $value;
			}
	...
	public function build($view, $data = array(), $output = FALSE)
	{
		...
		foreach($this->css as $key => $value)
		{
			...
			else
			{
				...
				{
					$css_path = $this->ci->config->base_url($this->assets_path . '/' . $key . '.css');
		...
		{
			if ($key == $value)
				$template_data['js_header'] .= "\t\t<script src=\"" . $this->ci->config->base_url($this->assets_path . '/' . $key . '.js') . "\"></script>\r\n";
			else
			{
		...
		{
			if ($key == $value)
				$template_data['js_footer'] .= "\t\t<script src=\"" . $this->ci->config->base_url($this->assets_path . '/' . $key . '.js') . "\"></script>\r\n";
			else
			{
